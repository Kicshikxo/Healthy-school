<template>
    <div class="flex flex-column gap-2 p-5 pt-2">
        ...
        <div ref="PDFTemplate" class="fixed pointer-events-none" :style="{ 'z-index': PDFTemplatePreview ? 'auto' : -1 }">
            <statistics-pdf-organization-template />
        </div>
    </div>
</template>
...
onMounted(() => {
    document.addEventListener('keydown', (event) => (PDFTemplatePreview.value = event.altKey))
    document.addEventListener('keyup', (event) => (PDFTemplatePreview.value = event.altKey))
})

const PDFTemplatePreview = ref(false)
const PDFTemplate = ref<HTMLElement>()
...
async function printPDF() {
    ...
    const pdf = await toPDF({ element: PDFTemplate.value! })
...
async function savePDF() {
    ...
    PDFLoading.value = false
    pdf.download(PDFFileName.value)
}
